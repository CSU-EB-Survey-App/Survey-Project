...
//   }
  ...
  const PrevArrow = ({ onClick }) => (
    ...
    <img className="slick-prev" 
        style={{height: "66px", width: "50px", left: "-45px", opacity: "0.5"}} src={SVGLeft} alt="Previous" onClick={onClick}/>
  );
  
  const NextArrow = ({ onClick }) => (
    <img className="slick-next" 
        style={{height: "66px", width: "50px", right: "-45px", opacity: "0.5"}} 
        src={SVGRight} alt="Previous" onClick={onClick}/>
  );
        ...
        <Fragment>
            {items.length < 1 ? <Typography variant="h5" style={{width: "100%", textAlign: "center"}}>{emptyItemsMessage}</Typography> : (
                ...
                <Grid item xs={12}>
                    ...
                    <div key={index}>
                        <NavLink to={`/${url}/${item._id}`} style={{textDecoration: "none", userSelect: "none"}}>
                            <Card sx={{minWidth: 200, minHeight: 200, }} style={{margin: "10px"}}>
                            <CardHeader
                                avatar={
                                <Avatar>
                                    <PersonIcon />
