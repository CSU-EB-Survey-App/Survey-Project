    ...
    describe("POST Register User Route: /api/v1/auth/register", () => {
      describe("Given the input fields ARE correct", () => {
        it("Should return a 200", async () => {
          // expect(true).toBe(true); // Good way to test function setup
          const id = `MN${Math.floor(Math.random() * 9999) + 0}`;
          const tempEmail = `stephen${
            Math.floor(Math.random() * 9999) + 0
          }@100.com`;
          await superT(Svr)
    ...
    describe("Given BAD token", () => {
      it("Should return a 400", async () => {
        await superT(Svr)
          ...
          .send({
            token:
              "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY0YjM2ZTdjYzVlZDdjOWU0ZGRlM2E5NyIsImlhdCI6MTY4OTQ4MDgyOCwiZXhwIjoxNjkyMDcyODI4fQ.d5Ka2eE-Ls8UHETFb2lkDbVVjjcwszFyGVkjPMM6Tyz",
          })
          .expect(400);
      });
    });
...
describe("Authentication", () => {
  describe("DELETE User Account Route: /api/v1/auth/delete", () => {
    describe("Given GOOD token", () => {
      it("Should return a 200", async () => {
        let user = await User.create({
          studentID: `KG${Math.floor(Math.random() * 9999) + 0}`,
          email: `unittesting${
            Math.floor(Math.random() * 9999) + 0
          }@pollstesting100.com`,
          password: "123456789",
        ...
        const jsonWebToken = await user.getSignedJwtToken();

        await superT(Svr)
          .delete(`/api/v1/auth/delete/${jsonWebToken}`)
          .expect(200);
      });
    });
  });
...
describe("Authentication", () => {
  describe("DELETE User Account Route: /api/v1/auth/delete", () => {
    describe("Given BAD token", () => {
      it("Should return a 400", async () => {
        const badUserToken =
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY0YmYyMDVhYWMxMGE3NmNjMTI5OTJkYSIsImlhdCI6MTY5MDI0NzI1OCwiZXhwIjoxNjkyODM5MjU4fQ.H28Z6LtQ4OM4iPzx6vzvzudsnZIGsdtG5JTqZeXRzRx";
        await superT(Svr)
          .delete(`/api/v1/auth/delete${badUserToken}`)
          .expect(404);
      });
    });
  });
